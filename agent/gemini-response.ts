 ...
 */
export function validateGeminiGenerateContentResponse(data: unknown): string[] {
  const errors: string[] = [];
...
/**
 * Safely extract generated text from a Gemini response with fallback.
 * Returns the generated text if valid, otherwise returns the provided fallback.
 * Never throws.
 */
export function extractGeminiGeneratedTextSafe(
  data: unknown,
  fallback: string,
): string {
  try {
    ...
    console.warn("Failed to extract Gemini response, using fallback:", error);
    return fallback;
  }
}
